...
namespace cango {
    ...
    struct result_code {
        ...
        /// @brief 检查错误代码是否代表结果为成功或警告，中间的错误有可能被掩盖
        [[nodiscard]] constexpr bool is_not_failed() const noexcept { return code >= success_code; }

        /// @brief 检查错误代码是否代表结果为警告，有被处理的错误，需要引起注意
